...
export default async function SegurosPatrimoniales() {
    const data = await getPageData("patrimoniales");
    const sections = data?.gutenberg_structure || [];

    // Extract Hero Section (First Section)
    const heroSection = sections[0];
    const heroBlocks = heroSection?.blocks || [];
    const heroTitle = heroBlocks.find((b: any) => b.type === "core/heading")?.content || "SEGUROS PATRIMONIALES";
    const heroDesc = heroBlocks.find((b: any) => b.type === "core/paragraph")?.content || "Los seguros patrimoniales protegen tus bienes más importantes frente a pérdidas, daños o eventos inesperados.";
    const heroBg = heroSection?.attributes?.style?.background?.backgroundImage?.url || "https://images.unsplash.com/photo-1542744173-8e7e53415bb0?q=80&w=2670&auto=format&fit=crop";
    ...
    const dynamicProducts = columns.map((col: any) => {
        ...
        return {
            ...
            image: imgBlock?.url || "https://images.unsplash.com/photo-1541888946425-d81bb19240f5?q=80&w=2670&auto=format&fit=crop",
