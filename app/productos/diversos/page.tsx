...
export default async function Diversos() {
    const data = await getPageData("diversos");
    const sections = data?.gutenberg_structure || [];

    // Extract Hero Section (First Section)
    const heroSection = sections[0];
    const heroBlocks = heroSection?.blocks || [];
    const heroTitle = heroBlocks.find((b: any) => b.type === "core/heading")?.content || "SEGUROS DIVERSOS";
    const heroDesc = heroBlocks.find((b: any) => b.type === "core/paragraph")?.content || "Protección para cada detalle. Coberturas específicas diseñadas para riesgos técnicos, operativos y financieros de tu empresa.";
    const heroBg = heroSection?.attributes?.style?.background?.backgroundImage?.url || "https://images.unsplash.com/photo-1497215728101-856f4ea42174?q=80&w=2670&auto=format&fit=crop";
    ...
    const dynamicProducts = columns.map((col: any) => {
        ...
        return {
            ...
            image: imgBlock?.url || "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=2670&auto=format&fit=crop",
